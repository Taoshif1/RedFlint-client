  ...
  <div className="flex items-center gap-1">
    {[1, 2, 3, 4, 5].map((star) => (
      <FaStar
        key={star}
        size={size}
        className={star <= Math.round(rating) ? "text-warning" : "text-base-content/20"}
      />
    ))}
  </div>
);
...
const ProductReviews = ({ productId, productTitle }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (comment.trim().length < 5) return toast.error("Please write a review");
  ...
  return (
    <section className="mt-16 border-t border-base-300 pt-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
        ...
        <button onClick={() => setShowForm((previous) => !previous)} className="btn btn-primary">
      ...
      <div className="card bg-base-200 border border-base-300 mb-8">
        <div className="card-body flex-row items-center gap-6">
          <div>
            <div className="text-5xl font-bold">{averageRating > 0 ? averageRating : "—"}</div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="card bg-base-100 border border-base-300 shadow-xl mb-10">
          <div className="card-body">
            <h3 className="text-2xl font-bold">Review {productTitle}</h3>

            <div className="alert bg-base-200 border-none mt-3">
              <span className="text-sm">
                Share your experience with this product. Your review will appear after approval.
              </span>
            </div>
            ...
            <div className="mt-4">
              ...
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  ...
                  >
                    <FaStar size={30} className={star <= rating ? "text-warning" : "text-base-content/20"} />
                  </button>
                ))}
              </div>
            ...
            <label className="form-control mt-4">
              ...
              />
              <span className="text-xs text-base-content/40 mt-2 text-right">{comment.length}/1000</span>
            </label>
      ...
      {loading ? (
        ...
        <div className="card bg-base-200 border border-base-300">
          <div className="card-body text-center py-12">
            ...
            <p className="text-base-content/60">Be the first to share your experience with this product.</p>
          </div>
        </div>
      ) : (
        <div className="grid md:grid-cols-2 gap-5">
          {reviews.map((review) => (
            <article key={review._id} className="card bg-base-100 border border-base-300">
              <div className="card-body">
                <div className="flex items-start justify-between gap-3">
                  <h3 className="font-bold text-lg">{review.customerName || "RedFlint Customer"}</h3>
                  <Stars rating={review.rating} />
                </div>

                <p className="text-base-content/80 leading-7 mt-3">{review.comment}</p>
